	...
	function TranslationAdder (insertUl)
	{
		...
		this.fields =  {
			lang: function (txt, error)
			{
				...
				if (txt == 'no')
										return error("Please use the code nb for Bokmål or nn for Nynorsk");
				if (txt == 'nds')
		...
		this.onsubmit = function (values, render)
		{
			...
			{
				wikitext = '{'+'{trreq|' + '{'+'{subst:#invoke:language utilities|lookup_language|' + values.lang + '|names}}}}';
				callRender();
			}
			else
			{
				wikitext = '{'+'{subst:#invoke:language utilities|lookup_language|' + values.lang + '|names}}: ' +  
					'{'+'{t|' + values.lang + '|' + (values.alt ? values.alt : values.word) +
				...
				langmetadata.hasWiktionaryWithEntry(values.lang, values.alt, function (result) {
					if (result)
						wikitext = wikitext.replace(/\{t\|/, '{t+|');
					else if (langmetadata.hasWiktionary(values.lang))
						wikitext = wikitext.replace(/\{t\|/, '{t-|');
					else
						wikitext = wikitext.replace(/\{t\|/, '{tø|');
					callRender();
				});
			}
			if (!this.balancer)
		...
		function resetElements ()
		{
			...
			thiz.elements.word.value = thiz.elements.nclass1.value = thiz.elements.nclass2.value = thiz.elements.tr.value = thiz.elements.alt.value = thiz.elements.qual.value = '';
			...
			function addEdit (edit, span)
			{
				editor.addEdit({
					'undo': function () 
					{
						...
						{
							var fields = ["lang","word","nclass1","nclass2","alt","qual","tr","sc"];
			...
			return function(text)
			{
				...
				if (findLanguage)
				{
					...
					if (trreq)
					{
						...
						if (ipos < 0 || ipos > endpos)
							ipos = text.indexOf('{'+'{trreq|{'+'{subst:#invoke:language utilities|lookup_language|'+findLangCode+'|names}}}}', stapos);
					}
					...
					if (!trreq || ipos < 0 || ipos > endpos )
					{
						...
						if (ipos < stapos || ipos > endpos)
							ipos = text.substr(stapos).search(RegExp('\\*[:*]? ?' + util.escapeRe(findLanguage) + ':')) + stapos;
						if (ipos < stapos || ipos > endpos)
							ipos = text.indexOf('{'+'{subst:#invoke:language utilities|lookup_language|'+findLangCode+'|names}}:', stapos);
					}
...
function LangMetadata ()
{
	...
	// FIXME: Is it possible to query this information directly from [[Module:languages]]?
		var metadata = {
			...
			aer:{sc:"Latn"},
			af:{g:["s","p"],haswikt:1,sc:"Latn"},
			ak:{haswikt:1},
			akk:{g:["m","f","m-p","f-p"],sc:"Xsux"},
			als:{haswikt:1},
			am:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Ethi"},
			an:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			ang:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			ar:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Arab"},
			arc:{g:["m","f","m-p","f-p"],sc:"Hebr"},
			are:{sc:"Latn"},
			arz:{alt:1,g:["m","f","m-p","f-p"],sc:"Arab"},
			as:{haswikt:1,sc:"Beng"},
			ast:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			av:{haswikt:1,sc:"Cyrl"},
			axm:{sc:"Armn"},
			ay:{haswikt:1},
			az:{g:["s","p"],haswikt:1,sc:["Latn","Cyrl","Arab"]},
			ba:{sc:"Cyrl"},
			bar:{sc:"Latn"},
			"bat-smg":{g:["m","f","m-p","f-p"],sc:"Latn"},
			be:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:["Cyrl","Latn"]},
			"be-x-old":{sc:"Cyrl"},
			bg:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Cyrl"},
			bh:{haswikt:1,sc:"Deva"},
			bhb:{sc:"Deva"},
			bi:{haswikt:1,sc:"Latn"},
			blt:{sc:"Tavt"},
			bm:{haswikt:1,sc:["Latn","Nkoo","Arab"]},
			bn:{haswikt:1,sc:"Beng"},
			bo:{haswikt:1,sc:"Tibt"},
			br:{g:["m","f"],haswikt:1,sc:"Latn"},
			ca:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			cdo:{sc:"Hani"},
			ch:{haswikt:1,sc:"Latn"},
			chr:{haswikt:1,sc:"Cher"},
			cjy:{sc:"Hani"},
			ckb:{sc:"Arab",wsc:"ku-Arab"},
			cmn:{wiktprefix:"zh",sc:"Hani"},
			co:{haswikt:1,sc:"Latn"},
			cpx:{sc:"Hani"},
			cr:{haswikt:1,sc:"Cans"},
			crh:{sc:"Latn"},
			cs:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			csb:{haswikt:1},
			cu:{g:["m","f","n","m-p","f-p","n-p"],sc:["Cyrs","Glag"]},
			cv:{sc:"Cyrl"},
			cy:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			czh:{sc:"Hani"},
			czo:{sc:"Hani"},
			da:{g:["c","n","c-p","n-p"],haswikt:1,sc:"Latn"},
			dax:{sc:"Latn"},
			de:{g:["m","f","n","p"],haswikt:1,sc:"Latn"},
			dhg:{sc:"Latn"},
			djb:{sc:"Latn"},
			dji:{sc:"Latn"},
			djr:{sc:"Latn"},
			...
			el:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Grek"},
			en:{g:["s","p"],haswikt:1,sc:"Latn"},
			eo:{g:["s","p"],haswikt:1,sc:"Latn"},
			es:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			et:{g:["s","p"],haswikt:1,sc:"Latn"},
			ett:{g:["s","p"],sc:"Ital"},
			eu:{g:["s","p"],haswikt:1,sc:"Latn"},
			fa:{haswikt:1,sc:"Arab",wsc:"fa-Arab"},
			fi:{g:["s","p"],haswikt:1,sc:"Latn"},
			fil:{sc:"Latn"},
			fj:{haswikt:1,sc:"Latn"},
			fo:{g:["m","f","n"],haswikt:1,sc:"Latn"},
			fr:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			frm:{g:["m","f","m-p","f-p"],sc:"Latn"},
			fro:{g:["m","f","m-p","f-p"],sc:"Latn"},
			fy:{haswikt:1,sc:"Latn"},
			ga:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			gan:{sc:"Hani"},
			gd:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			gez:{sc:"Ethi"},
			gl:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			gmy:{sc:"Linb"},
			gn:{haswikt:1},
			gnn:{sc:"Latn"},
			got:{g:["m","f","n","m-p","f-p","n-p"],sc:"Goth"},
			grc:{g:["m","f","n","m-p","f-p","n-p"],sc:"Grek",wsc:"polytonic"},
			...
			hak:{sc:"Hani"},
			har:{sc:"Ethi"},
			he:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Hebr"},
			hi:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Deva"},
			hif:{sc:["Latn","Deva"]},
			hit:{sc:"Xsux"},
			hsb:{haswikt:1},
			hsn:{sc:"Hani"},
			hu:{g:["s","p"],haswikt:1,sc:"Latn"},
			hy:{haswikt:1,sc:"Armn"},
			ia:{haswikt:1,sc:"Latn"},
			id:{haswikt:1,sc:"Latn"},
			ie:{haswikt:1,sc:"Latn"},
			ik:{haswikt:1},
			ike:{sc:"Cans"},
			ikt:{sc:"Cans"},
			io:{haswikt:1},
			is:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			it:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			iu:{haswikt:1,sc:"Cans"},
			ja:{haswikt:1,sc:"Jpan"},
			jay:{sc:"Latn"},
			jbo:{haswikt:1,sc:"Latn"},
			jv:{haswikt:1},
			ka:{haswikt:1,sc:"Geor"},
			khb:{sc:"Talu"},
			kjh:{sc:"Cyrl"},
			...
			la:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			lb:{haswikt:1},
			lez:{sc:"Cyrl"},
			li:{haswikt:1},
			ln:{haswikt:1},
			lo:{haswikt:1,sc:"Laoo"},
			lt:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			lv:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			mg:{haswikt:1},
			mh:{haswikt:1},
			mi:{g:0,haswikt:1,sc:"Latn"},
			mk:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Cyrl"},
			...
			mt:{g:["m","f"],haswikt:1,sc:"Latn"},
			mwp:{sc:"Latn"},
			my:{haswikt:1,sc:"Mymr"},
			na:{haswikt:1},
			nah:{haswikt:1},
			nan:{wiktprefix:"zh-min-nan",sc:"Hani"},
			nb:{g:["m","f","n","m-p","f-p","n-p"],wiktprefix:"no",sc:"Latn"},
			"nds-de":{g:["m","f","n","m-p","f-p","n-p"],wiktprefix:"nds",sc:"Latn"},
			"nds-nl":{g:["m","f","n","m-p","f-p","n-p"],wiktprefix:"nds",sc:"Latn"},
			ne:{haswikt:1,sc:"Deva"},
			nl:{g:["m","f","n","p"],haswikt:1,sc:"Latn"},
			nn:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			no:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			non:{g:["m","f","n","m-p","f-p","n-p"],sc:"Latn"},
			oc:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			om:{haswikt:1},
			...
			pdt:{g:["m","f","n","m-p","f-p","n-p"],wiktprefix:"nds",sc:"Latn"},
			peo:{sc:"Xpeo"},
			phn:{sc:"Phnx"},
			pi:{haswikt:1},
			pjt:{sc:"Latn"},
			pl:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			ps:{haswikt:1,sc:"Arab",wsc:"ps-Arab"},
			pt:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			qu:{haswikt:1},
			rit:{sc:"Latn"},
			rm:{g:["m","f"],haswikt:1,sc:"Latn"},
			rn:{haswikt:1},
			ro:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:["Latn","Cyrl"]},
			"roa-rup":{haswikt:1},
			ru:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Cyrl"},
			ruo:{g:["m","f","n","m-p","f-p","n-p"],sc:"Latn"},
			rup:{g:["m","f","n","m-p","f-p","n-p"],wiktprefix:"roa-rup",sc:"Latn"},
			ruq:{g:["m","f","n","m-p","f-p","n-p"],sc:"Latn"},
			rw:{haswikt:1,sc:"Latn"},
			sa:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Deva"},
			sah:{sc:"Cyrl"},
			sc:{haswikt:1},
			scn:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Latn"},
			sco:{sc:"Latn"},
			sd:{haswikt:1,sc:"Arab",wsc:"sd-Arab"},
			sg:{haswikt:1},
			sh:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:["Latn","Cyrl"]},
			si:{haswikt:1,sc:"Sinh"},
			simple:{haswikt:1,sc:"Latn"},
			sk:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			sl:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Latn"},
			sm:{haswikt:1},
			sn:{haswikt:1},
			so:{haswikt:1},
			spx:{sc:"Ital"},
			sq:{g:["m","f"],haswikt:1,sc:"Latn"},
			ss:{haswikt:1},
			st:{haswikt:1},
			su:{haswikt:1},
			sux:{sc:"Xsux"},
			sv:{g:["c","n","c-p","n-p"],haswikt:1,sc:"Latn"},
			sw:{nclass:1,haswikt:1,sc:"Latn"},
			syc:{sc:"Syrc"},
			syr:{sc:"Syrc"},
			ta:{haswikt:1,sc:"Taml"},
			tdd:{sc:"Tale"},
			te:{haswikt:1,sc:"Telu"},
			tg:{haswikt:1,sc:"Cyrl"},
			th:{haswikt:1,sc:"Thai"},
			ti:{haswikt:1,sc:"Ethi"},
			tig:{sc:"Ethi"},
			tiw:{sc:"Latn"},
			tk:{haswikt:1,sc:"Latn"},
			tl:{haswikt:1,sc:["Latn","Tglg"]},
			tmr:{sc:"Hebr"},
			tn:{haswikt:1},
			to:{haswikt:1},
			tpi:{haswikt:1,sc:"Latn"},
			tr:{g:["s","p"],haswikt:1,sc:"Latn"},
			ts:{haswikt:1},
			tt:{haswikt:1,sc:"Cyrl"},
			tw:{haswikt:1},
			ug:{haswikt:1,sc:"Arab",wsc:"ug-Arab"},
			uga:{sc:"Ugar"},
			uk:{g:["m","f","n","m-p","f-p","n-p"],haswikt:1,sc:"Cyrl"},
			ulk:{sc:"Latn"},
			ur:{g:["m","f","m-p","f-p"],haswikt:1,sc:"Arab",wsc:"ur-Arab"},
			uz:{haswikt:1,sc:"Latn"},
			vec:{haswikt:1,sc:"Latn"},
			vi:{haswikt:1,sc:"Latn"},
			vls:{g:["m","f","n","m-p","f-p","n-p"],sc:"Latn"},
			vo:{haswikt:1},
			wa:{haswikt:1},
			wbp:{sc:"Latn"},
			wo:{haswikt:1},
			...
			xcr:{sc:"Cari"},
			xfa:{sc:"Ital"},
			xh:{alt:1,nclass:1,haswikt:1,sc:"Latn"},
			xlc:{sc:"Lyci"},
			xld:{sc:"Lydi"},
			xlu:{sc:"Xsux"},
			xno:{g:["m","f","m-p","f-p"],sc:"Latn"},
			xrr:{sc:"Ital"},
			xst:{sc:"Ethi"},
			xum:{sc:"Ital"},
			xve:{sc:"Ital"},
	...
	var nesting = {
		...
		osp:'Spanish',
		owl:'Welsh',wlm:'Welsh',
		kmr:'Kurdish/Kurmanji',ckb:'Kurdish/Sorani',
	...
	var altForm = {
		...
		os: {from: "Ææ", to: "Ӕӕ"}, /** Cyrillic to Roman **/
